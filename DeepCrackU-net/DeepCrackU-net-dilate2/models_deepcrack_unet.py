...
class SegmentNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x9 = self.layer9(x8)
        x10 = self.layer10(torch.cat((x7, x9), 1))
        x11 = self.layer11(x10)
        x12 = self.layer12(torch.cat((x5, x11), 1))
        x13 = self.layer13(x12)
        x14 = self.layer14(torch.cat((x3, x13), 1))
        x15 = self.layer15(x14)
        x16 = self.layer16(torch.cat((x1, x15), 1))
